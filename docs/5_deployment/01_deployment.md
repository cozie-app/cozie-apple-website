...
Once, Cozie is installed, make sure the participant explicitly responds to the notification permission prompt. Then the settings can be changed for the participant. This can also be accelerated with a [QR code](qr_code).
...
* Send test push notification, i.e., confirm that the participant sees and interacts with the push notification (Make sure focus modes are turned off)
